...
template <class T>
std::string fmt(const T &data, string open = "[", string close = "]",
                string sep = ", ") {
...
ostream &operator<<(ostream &os, Client *cli) { return os << cli->str(); }
...
struct Bank {
  ...
  Bank(int teller_size) { tellers = vector<Client *>(teller_size, nullptr); }
...
ostream &operator<<(ostream &os, Bank &bank) { return os << bank.str(); }
