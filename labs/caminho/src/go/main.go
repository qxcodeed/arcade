...
func search(grid [][]rune, startPos Pos, endPos Pos) {
	...
	for !queue.IsEmpty() {
		...
		for _, vizinho := range atual.getNeig() {
			if !visited[vizinho] && grid[vizinho.l][vizinho.c] != '#' {
				visited[vizinho] = true
				before[vizinho] = atual
				queue.Enqueue(vizinho)
			}
		}
	}
	// Marca todos os nós do início até o fim
	for pos := endPos; ; pos = before[pos] {
		grid[pos.l][pos.c] = '.'
		if pos == startPos {
			break
		}
