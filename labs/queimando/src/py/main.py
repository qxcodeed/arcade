...
def queimar(mat: list[list[str]], l: int, c: int) -> None: # Usando 'list' diretamente
    ...
    # pass
    # @DROP
    stack: list[Pos] = [] # Usando 'list' diretamente

    # Verifica se a posição inicial é uma árvore (#) antes de começar
    if not inside(mat, Pos(l, c)) or mat[l][c] != '#':
        return

    mat[l][c] = 'o'  # Marca a primeira árvore como queimada
    stack.append(Pos(l, c)) # Adiciona à pilha

    while stack: # Equivalente a 'while not stack.IsEmpty()'
        current_pos: Pos = stack[-1] # Pega o elemento do topo sem remover

        found_unburned_neighbor: bool = False
        for neighbor in get_neib(current_pos):
            # Se o vizinho estiver dentro dos limites e for uma árvore (#)
            if inside(mat, neighbor) and mat[neighbor.l][neighbor.c] == '#':
                mat[neighbor.l][neighbor.c] = 'o' # Queima o vizinho
                stack.append(neighbor) # Adiciona o vizinho queimado à pilha
                found_unburned_neighbor = True
                break # Pega o primeiro vizinho queimável e avança
        
        if not found_unburned_neighbor:
            stack.pop() # Se não encontrou vizinhos queimáveis, remove da pilha (backtrack)
